namespace BackEnd.Controllers
{
    ...
    /// <summary>Creates a new <c>ProducerCrmVisitController</c> for managing requests to the  web service. Inherits from <see cref="Controller"/>.
    /// </summary>
    public class ProducerCrmVisitController : Controller
    {
        /// <summary>The web service connection.
        /// </summary>
        private readonly ProducerCrmVisitContext _webProducerCrmDatabaseConnection = new ProducerCrmVisitContext();
    ...
    name += " (BETA)";
        ...
        /// <remarks>Opens the <see cref="BackEnd.Models.ProducerCrmVisitContext"/> connection, queries the <see cref="VisitXReason"/> table for the given
    ...
    /// <summary>Creates a new <c>EnterpriseController</c> for managing requests to the  web service. Inherits from <see cref="Controller"/>.
    /// </summary>
    public class EnterpriseController : Controller
    {
        /// <summary>The web service connection.
        /// </summary>
        private readonly EnterpriseContext _webEnterpriseDatabaseConnection = new EnterpriseContext();
        ...
        public ActionResult Index()
        {
            ViewBag.VisitReportCount = _webEnterpriseDatabaseConnection.StoredProducerVisitReports.Count();
            ViewBag.UserCount = _webEnterpriseDatabaseConnection.UserIdentities.Count();
            ViewBag.UniqueUsers = _webEnterpriseDatabaseConnection.UserIdentities.Distinct().Count();
            //ViewBag.EntityNumber = _webMemberDatabaseConnection.Database.SqlQuery<string>("mySpName {0}, {1}, {2}", new object[] { param1, param2, param3 });
            //ViewBag.EntityNumber = _webMemberDatabaseConnection.Database.SqlQuery<En>("SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_NAME LIKE '%{0}%'", new object[] { "Call" });
        ...
        private string DatabaseSource()
        {
            ...
            source = _webEnterpriseDatabaseConnection.Database.Connection.DataSource;
	...
	source = "Production";
        ...
        private string DatabaseName()
        {
            ...
            name = _webEnterpriseDatabaseConnection.Database.Connection.Database;
    ...
    name += " (BETA)";
        ...
        public ActionResult Recent(string id)
        {
            ...
            var storedProducerVisitReports = _webEnterpriseDatabaseConnection.StoredProducerVisitReports
                .Where(visitReport => visitReport.MemberNumber.Contains(id))
                .OrderByDescending(visitReport => visitReport.VisitDate)
                .Take(quantity)
                .ToList();
            ...
            var reportListItems = storedProducerVisitReports.Select(storedProducerVisitReport =>
            {
                var producerVisitReport = Hydrated(storedProducerVisitReport);
                // FixMe: change this to filter using the user's email address instead of userID.
                var userID = _webEnterpriseDatabaseConnection.UserIdentities
                    .FirstOrDefault(uid => uid.DeviceID == storedProducerVisitReport.UserID);
                return new ReportListItem
                {
                    ID = storedProducerVisitReport.ID,
        ...
        public ActionResult All(string id)
        {
            var storedProducerVisitReports = _webEnterpriseDatabaseConnection.StoredProducerVisitReports.Where(visitReport => visitReport.MemberNumber == id)
                .OrderByDescending(visitReport => visitReport.VisitDate)
                .ToList();
            ...
            var reportListItems = storedProducerVisitReports.Select(storedProducerVisitReport =>
            {
                ...
                {
                    ID = storedProducerVisitReport.ID,
                    UserEmail = _webEnterpriseDatabaseConnection.UserIdentities.First(uid => uid.DeviceID == storedProducerVisitReport.UserID).UserEmail,
                    MemberNumber = producerVisitReport.MemberNumber,
                    Local = false,
                    PrimaryReasonCode = producerVisitReport.ReasonCodes[0],
                    VisitDate = producerVisitReport.VisitDate,
        ...
        /// <summary>Opens the <see cref="_webEnterpriseDatabaseConnection"/>, adds a <see cref="ReasonCode"/>[], and 
        /// returns a <see cref="ProducerVisitReport"/>.
        /// </summary>
        /// <param name="storedProducerVisitReport">A <see cref="StoredProducerVisitReport"/>.</param>
        /// <returns>A <see cref="ProducerVisitReport"/> based on a <see cref="StoredProducerVisitReport"/>.</returns>
        /// <remarks>Opens the <see cref="BackEnd.Models.EnterpriseContext"/> connection, queries the <see cref="VisitXReason"/> table for the given
        /// <see cref="StoredProducerVisitReport"/> ID, matches the VisitXReason.ReasonIDs against the <see cref="ReasonCode"/> table
        /// to get a <see cref="ReasonCode"/>[], and returns the StoredProducerVisitReport.Hydrate(reasonCodes), aka a <see cref="ProducerVisitReport"/>.</remarks>
        private ProducerVisitReport Hydrated(StoredProducerVisitReport storedProducerVisitReport)
        {
            var vxrs = _webEnterpriseDatabaseConnection.VisitXReasons.Where(vxr => vxr.VisitID == storedProducerVisitReport.ID).ToList();
            var ids = vxrs.Select(vxr => vxr.ReasonID).ToList();
            var rcs = _webEnterpriseDatabaseConnection.ReasonCodes.Where(rc => ids.Contains(rc.ID)).ToArray();
            return storedProducerVisitReport.Hydrate(rcs);
        }
        ...
        public ActionResult Log(ProducerVisitReport report)
        {
            report.ID = 0;
            var storedProducerVisitReport = new StoredProducerVisitReport(report);
            _webEnterpriseDatabaseConnection.StoredProducerVisitReports.Add(storedProducerVisitReport);
            _webEnterpriseDatabaseConnection.SaveChanges();
            if (report.ReasonCodes != null)
            {
                foreach (var rc in report.ReasonCodes)
                {
                    _webEnterpriseDatabaseConnection.VisitXReasons.Add(new VisitXReason { ReasonID = rc.ID, VisitID = storedProducerVisitReport.ID });
                }
                _webEnterpriseDatabaseConnection.SaveChanges();
            }
            return Content("Success");
        ...
        public ActionResult Identity(UserIdentity report)
        {
            _webEnterpriseDatabaseConnection.UserIdentities.Add(report);
            _webEnterpriseDatabaseConnection.SaveChanges();
            return Content("Success");
        }
        ...
        public ActionResult Reasons(string id)
        {
            ...
            // check remote
            if (!_webEnterpriseDatabaseConnection.ReasonCodes.Any())
            {
                reasonCodeList.Add(new ReasonCode { Name = "VstCntrllr: no ReasonCodes on web dB", Code = -1 });
            }
            else
            {
                reasonCodeList = _webEnterpriseDatabaseConnection.ReasonCodes.ToList();
            }
        ...
        public ActionResult Report(string id)
        {
            return Json(Hydrated(_webEnterpriseDatabaseConnection.StoredProducerVisitReports.Find(int.Parse(id))), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult CallTypes(string id)
        {
            ...
            if (!_webEnterpriseDatabaseConnection.CallTypes.Any())
            {
                callTypeList.Clear();
                callTypeList.Add(new CallType { Name = "VstCntrllr: no CallTypes on web dB" });
            }
            ...
            {
                callTypeList = _webEnterpriseDatabaseConnection.CallTypes.ToList();
            }
        ...
        public ActionResult EmailRecipients(string id)
        {
            ...
            if (!_webEnterpriseDatabaseConnection.EmailRecipients.Any())
            {
                objectList.Clear();
                objectList.Add(new EmailRecipient { DisplayName = "VstCntrllr: no EmailRecipients on web dB" });
            }
            ...
            {
                objectList = _webEnterpriseDatabaseConnection.EmailRecipients.ToList();
            }
        ...
        protected override void Dispose(bool disposing)
        {
            _webEnterpriseDatabaseConnection.Dispose();
            base.Dispose(disposing);
        }
    }
